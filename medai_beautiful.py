...
[data-testid="stSidebar"] * { color: #1A3D2E !important; }
...
[data-testid="stChatMessage"] p { color: #1A3D2E !important; font-size: 14px !important; line-height: 1.7 !important; }
[data-testid="stChatMessage"] h1,
[data-testid="stChatMessage"] h2,
[data-testid="stChatMessage"] h3 { color: #0D6E4E !important; }
...
.stSuccess { background: #E8F5EF !important; border: 1.5px solid #1D9E75 !important; border-radius: 10px !important; color: #0D6E4E !important; }
.stWarning { background: #FFF8E7 !important; border: 1.5px solid #F59E0B !important; border-radius: 10px !important; }
.stError { background: #FEF2F2 !important; border: 1.5px solid #EF4444 !important; border-radius: 10px !important; }
.stInfo { background: #EFF6FF !important; border: 1.5px solid #3B82F6 !important; border-radius: 10px !important; }
[data-testid="stMetric"] { background: white !important; border: 1px solid #D1EAE0 !important; border-radius: 14px !important; padding: 16px 20px !important; }
[data-testid="stMetricLabel"] { color: #6B9E8A !important; }
...
.medai-hero p { font-size: 15px !important; color: rgba(255,255,255,0.85) !important; margin: 0 !important; }
.medai-badge {
    display: inline-flex;
    align-items: center;
    gap: 6px;
...
.stat-number { font-family: 'Sora', sans-serif; font-size: 28px; color: #1D9E75; font-weight: 700; }
.stat-label { font-size: 12px; color: #6B9E8A; text-transform: uppercase; letter-spacing: 0.06em; margin-top: 4px; font-weight: 500; }

.sidebar-brand { text-align: center; padding: 20px 0 16px; }
.sidebar-brand .icon { font-size: 42px; display: block; margin-bottom: 6px; }
.sidebar-brand h2 { font-family: 'Sora', sans-serif !important; font-size: 22px !important; font-weight: 700 !important; color: #1D9E75 !important; margin: 0 0 2px !important; }
.sidebar-brand p { font-size: 11px !important; color: #6B9E8A !important; text-transform: uppercase !important; letter-spacing: 0.1em !important; margin: 0 !important; }

.user-card { background: linear-gradient(135deg, #E8F5EF, #F0FAF5); border: 1.5px solid #A8D5BE; border-radius: 12px; padding: 12px 16px; margin-bottom: 8px; }
.user-card .label { font-size: 11px; color: #6B9E8A; font-weight: 500; text-transform: uppercase; letter-spacing: 0.06em; }
.user-card .name { font-size: 15px; font-weight: 600; color: #1D9E75; margin-top: 2px; }

.nav-label { font-size: 11px; color: #6B9E8A; text-transform: uppercase; letter-spacing: 0.08em; font-weight: 600; margin: 12px 0 6px; }

.source-item { background: #F8FCF9; border: 1px solid #D1EAE0; border-radius: 8px; padding: 8px 12px; margin: 4px 0; font-size: 13px; }
.source-item a { color: #1D9E75 !important; text-decoration: none; font-weight: 500; }

.page-header { background: white; border: 1px solid #D1EAE0; border-radius: 16px; padding: 24px 28px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); }
.page-header h2 { font-family: 'Sora', sans-serif !important; font-size: 24px !important; font-weight: 700 !important; color: #0D6E4E !important; margin: 0 0 4px !important; }
.page-header p { font-size: 14px !important; color: #6B9E8A !important; margin: 0 !important; }
...
.footer-credit { text-align: center; padding: 16px 0 8px; border-top: 1px solid #D1EAE0; margin-top: 16px; }
.footer-credit p { font-size: 11px !important; color: #A8C5B8 !important; margin: 2px 0 !important; }
</style>
...
Be detailed, accurate, helpful. Use clear formatting.
...
def process_voice(audio):
    """Convert audio to text — any language"""
...
if st.session_state.page == "chat":
    ...
    <div class="medai-hero">
        <h1>🏥 MedAI</h1>
        <p>Medical Super Intelligence — Ask anything, get expert answers instantly</p>
        <div class="medai-badge">🌍 Searches 15+ sources · 50+ languages · Free Forever</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ── Voice Input — clean section ──
    st.markdown("""
    <div class="voice-section">
        <div>
            <div class="voice-label">🎤 Voice Input</div>
            <div class="voice-hint">Click mic → speak in ANY language → AI answers</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown('<div class="page-header"><h2>📸 Image Analysis</h2><p>Upload any medical image — body part, skin, scan, food, diagram</p></div>', unsafe_allow_html=True)
    uploaded = st.file_uploader("Upload medical image", type=["jpg","jpeg","png","webp"])
    question = st.text_input("What do you want to know?", "Give full medical analysis")
    if uploaded:
    ...
    st.markdown('<div class="page-header"><h2>📄 PDF Analysis</h2><p>Upload any medical PDF — reports, prescriptions, research papers</p></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="page-header"><h2>👤 Health Profile</h2><p>MedAI personalizes answers based on your profile</p></div>', unsafe_allow_html=True)
    if not st.session_state.current_user:
