...
with st.sidebar:
    ...
    st.markdown('<div class="nav-label">💡 Quick Questions</div>', unsafe_allow_html=True)
    st.markdown("""
    <style>
    .suggest-popup {
        display:none;
    ...
    .suggest-popup.open { display:block; }
    .suggest-pill {
        display:block;
        background:#F0F7F4;
    ...
    <button id="toggleBtn" onclick="toggleSuggest()">💡 Show suggested questions ▼</button>
    <div class="suggest-popup" id="suggestPopup">
        <button class="suggest-pill" onclick="sendQ('What causes chest pain?')">What causes chest pain?</button>
        <button class="suggest-pill" onclick="sendQ('Symptoms of diabetes?')">Symptoms of diabetes?</button>
        <button class="suggest-pill" onclick="sendQ('How does cancer develop?')">How does cancer develop?</button>
        <button class="suggest-pill" onclick="sendQ('Signs of heart attack?')">Signs of heart attack?</button>
        <button class="suggest-pill" onclick="sendQ('How to boost immunity?')">How to boost immunity?</button>
        <button class="suggest-pill" onclick="sendQ('What causes headache?')">What causes headache?</button>
        <button class="suggest-pill" onclick="sendQ('How to treat depression?')">How to treat depression?</button>
        <button class="suggest-pill" onclick="sendQ('Signs of stroke?')">Signs of stroke?</button>
        <button class="suggest-pill" onclick="sendQ('What is Alzheimers disease?')">What is Alzheimer's?</button>
        <button class="suggest-pill" onclick="sendQ('How does chemotherapy work?')">How does chemotherapy work?</button>
    </div>
    <script>
    function toggleSuggest() {
        var popup = document.getElementById('suggestPopup');
        var btn = document.getElementById('toggleBtn');
        popup.classList.toggle('open');
        if (popup.classList.contains('open')) {
            btn.innerHTML = '💡 Hide questions ▲';
        } else {
            btn.innerHTML = '💡 Show suggested questions ▼';
        }
    }
    function sendQ(q) {
        ...
        document.getElementById('toggleBtn').innerHTML = '💡 Show suggested questions ▼';
    }
    </script>
    """, unsafe_allow_html=True)
...
if st.session_state.page == "chat":
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
        <button onclick="startVoice()" id="voiceBtn"
            style="background:#1D9E75;color:white;border:none;border-radius:50px;
            padding:10px 22px;font-size:14px;font-weight:600;cursor:pointer;
            display:flex;align-items:center;gap:8px;font-family:Inter,sans-serif;
            box-shadow:0 4px 14px rgba(29,158,117,0.3);transition:all 0.2s;">
            🎤 Speak in any language
        </button>
        <span id="voiceStatus" style="font-size:13px;color:#6B9E8A;font-family:Inter,sans-serif;"></span>
    </div>
    ...
    function startVoice() {
        ...
        if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
            status.textContent = '⚠️ Voice not supported. Use Chrome browser!';
        ...
        recognition.onstart = function() {
            ...
            status.textContent = 'Speak now in ANY language...';
            status.style.color = '#EF4444';
        };
        ...
        recognition.onresult = function(event) {
            ...
            if (event.results[event.results.length-1].isFinal) {
                ...
                // Also try putting it in the chat input
        ...
        recognition.onerror = function(event) {
            ...
            status.style.color = '#EF4444';
            isListening = false;
            btn.innerHTML = '🎤 Speak in any language';
