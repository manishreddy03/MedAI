...
if st.session_state.page == "chat":
    ...
    .voice-btn:hover { background: #178A64; transform: translateY(-1px); }
    .voice-status { font-size: 13px; color: #6B9E8A; font-family: Inter, sans-serif; margin-left: 10px; }
    </style>
    <button class="voice-btn" id="voiceBtn" onclick="startVoice()">🎤 Speak in any language</button>
    <span class="voice-status" id="voiceStatus"></span>
    <script>
    var recognition; var isListening = false;
    function startVoice() {
        var btn = document.getElementById('voiceBtn');
        var status = document.getElementById('voiceStatus');
        if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
            status.textContent = '⚠️ Use Chrome for voice!'; return;
        }
        if (isListening) { recognition.stop(); return; }
        recognition = new (window.SpeechRecognition || window.webkitSpeechRecognition)();
        recognition.continuous = false; recognition.interimResults = true; recognition.lang = '';
        ...
        recognition.onresult = function(e) {
            ...
            if (e.results[e.results.length-1].isFinal) {
                ...
                if (ta) {
                    ...
                    setTimeout(function() { ta.dispatchEvent(new KeyboardEvent('keydown', {key:'Enter',code:'Enter',keyCode:13,bubbles:true})); }, 400);
                }
            }
        };
        recognition.onerror = function(e) {
            status.textContent = '⚠️ ' + e.error + ' — try again';
            isListening = false; btn.textContent = '🎤 Speak in any language'; btn.style.background = '#1D9E75';
        };
